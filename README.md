...
The Market Data WebSocket (MDWS) platform is a distributed system designed for ingesting, processing, and distributing real-time market data (Crypto, Forex, Equities) to clients. It leverages a microservices approach with specialized components for each stage of the data pipeline.

Key architectural principles include dynamic upstream subscriptions, high-throughput message routing, and scalable WebSocket server pods.
...
For a detailed data flow and component breakdown, please refer to the specific documentation for each service.

## Technology Stack

The MDWS platform is built using a diverse set of technologies, each chosen for its specific role in ensuring performance, scalability, and reliability:
